...
### 12b. experiment template の scaffold (default_conditions / scan_axis / child_template)
**規模**: 設計 1h + 実装 2〜3h

#12a (#63) で measurement template の result_fields による auto-fill が
入った。次は親 record (実験全体) 用の scaffold:

- `default_conditions: dict[str, Any]` — template に値も持たせる
  (operator / batch_no / deposition_method など実験ごとに固定の値)
- `scan_axis_field: str | None` — scan 軸を宣言 (Web UI の散布図 X 軸候補で優先)
- `default_child_template: str | None` — 子の measurement template
- `series.scaffold(rf_power_W=[20, 30, 40, 50, 60])` — 子 record を一括生成

これで script 実験 (1 record で conditions + results) と、
親子構造の scan 実験 (親で scan 軸、子で個別測定) を template 1 個で
表現可能になる。

agent teams での議論を先にやる予定 (前回の results / file API 議論と
同じ流れ)。

### 14. CI に frontend の build / type check を追加
**規模**: 30 分

PR #62 → #64 で発覚した課題: 現状の CI (`.github/workflows/ci.yml`) は
Python 側だけ (ruff / pytest / mypy) で frontend を検証していない。
TypeScript の strict check は **Deploy frontend** (Docker build 内の
`next build`) でしか走らないため、**マージしてから build 失敗する**
パターンを繰り返している (PR #62 で実際に発生)。

対策:

- `ci.yml` に `frontend` job を足す:
  - `cd platform/frontend && npm ci && npx tsc --noEmit && npm run lint`
  - or `npm run build` で本番ビルド相当を回す (こちらの方が確実)
- paths trigger は `platform/frontend/**` と共通 (`src/labvault/` も
  schema を変えると frontend 型に影響するので `src/` も入れる方が安全)
- node setup は `actions/setup-node@v4` で Node 20 (Deploy と揃える)

これで `next build` 落ちはマージ前に検知できる。

### 13b. Web UI: results card の auto-fill 由来表示
**規模**: 30〜60 分

#63 で template の result_fields から unit / description が auto-fill
されるようになった。Web UI 側で:

- 「template 由来 unit/desc」と「手動入力 unit/desc」を視覚的に区別
  (例: template 由来は灰チップ、手動は青チップ、または hover ヒント)
- record 詳細で template 名を明示し、result_fields のリストにリンク

LLM 解析品質を底上げするため、UI 上で「この値は template に登録された
正規 field」「これは自由入力 field」が分かるようにしたい。
...
| PR | 内容 |
|---|---|
| #54 | リモート MCP Phase 4: ローカル `labvault mcp` を「上級者向け 3 ケース (オフライン / 装置 PC dev / MCP ツール開発)」と位置付け直す docs 反映 (README / CLI docstring / instrument_pc_setup / auth_design / 設計メモ完了マーカー) |
| #55 | ファイル DL の防御強化: frontend `downloadAuthed` で `_t=${Date.now()}` cachebust 付与 (二重保険) + CLAUDE.md (project + backend) に「エラーレスポンスは `Cache-Control: no-store` 必須」「症状が直らない時は Network タブの `(from disk cache)` を疑う」を追記 |
| #56 | AR repo cleanup ポリシー設計 docs (`docs/ar_cleanup_policy.md`)。AR 組み込み cleanup policy で「直近 10 version 保持 + 365 日経過削除」、dry-run → 本適用 の手順。実 apply は別途運用判断 |
| #57 | AR grant retry endpoint + admin UI button。`POST /api/admin/users/{email}/ar/grant` (super-admin or 対象 user 所属 team の admin)、Firestore に `ar_granted` を persist、UserCard に「AR 未付与」badge + 「再付与」ボタン。backend test 6 件 |
| #58 | backend endpoint-level test 拡充 (+14): `test_auth_endpoints.py` 新規 (request_access / welcome_acknowledged の 9 件) + `test_admin_authz.py` に Business rules section (自己 deactivate / 最後の super-admin / 最後の team remove 等の 5 件)。conftest に AuthenticatedUser fixture と Slack 通知 capture を追加 |
| #59 | Record file API を役割別に分割: `add_file(path)` / `add_bytes(name, data)` / `add_object(name, obj)` / `put(target, name=)` の 4 method 追加 + `_store_bytes` ヘルパに統一。旧 `add` / `save` は alias 残置 (将来 minor で DeprecationWarning 予定、削除は v2.0 でも行わない)。tests +37、cookbook / README / onboarding / examples 書き換え、stale `add_ref(size_gb=)` も `size_bytes=` に修正 |
| #60 | results に flat-scalar 規約を hard error で強制 (dict 禁止 / list 32 要素以下 / 値 100KB / 合計 500KB)。違反は `ValidationError` で rollback、既存 record の load は graceful。`DataRef.original_type` ("ndarray"/"figure"/"dataframe"/"dict"/"list"/"str"/"bytes") を `add_object` 経路で自動付与し、Web UI / MCP / LLM が拡張子推測でなく metadata から判別可能に。tests +36 件、docs / examples / CLAUDE.md / CHANGELOG 更新 |
