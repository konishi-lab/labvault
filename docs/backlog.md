...
### 11. `labvault doctor --check-results` (results 規約の棚卸し)
**規模**: 30〜60 分

#60 で results に flat 規約 (dict 禁止 / list 32 / size guard) を強制した。
新規書き込みは規約に従うが、既存 Firestore record に dict 入り results が
何件あるかは未調査。`labvault doctor --check-results` で:

- Firestore を scan して results に dict / 長 list / サイズ超過がある
  record を列挙
- 件数と key 名を報告 (CSV / JSON 出力)
- 修正提案: flat 展開後の key 名と単位の sketch を出す

実 migration スクリプト (`--apply`) は別フェーズで判断。

### 12. template の `recommended_results` (flat key + unit + description)
**規模**: 設計 30 分 + 実装 1〜2 時間

#60 の規約が「単位 / description 付きの tuple 記法を推奨」だが、
ユーザーが毎回書くのは負担。template (XRD/SEM/SQUID/TEM/Raman) に
`recommended_results: [{key, unit, description}, ...]` を持たせて:

- `rec.results["peak_2theta_deg"] = 28.4` のように値だけ書けば
  template 側で単位 + description が補完される
- doctor が「template 紐付き record で recommended_results に未入力の
  key があれば」を警告
- Web UI の results card で「補完が効いた key」を視覚的に区別

LLM 解析品質を底上げする要。

### 13. Web UI: results card / file list を `original_type` 反映に
**規模**: 30〜60 分

#60 で `DataRef.original_type` ("ndarray" / "figure" / "dataframe" / etc.)
が attach されるようになった。Web UI 側で:

- file list に小バッジ表示 (Figure / Spectrum / DataFrame 等)
- 拡張子推測 fallback を残しつつ、`original_type` があればそれ優先
- 詳細プレビュー (`data_preview`) で original_type に応じたレンダリング
  分岐 (例: ndarray は数値統計、figure は image inline)
