...
| PR | 内容 |
|---|---|
| #84 | 🔥 S1 Phase 1A foundation: 別チーム record 共有機能の基盤を 1 PR で着地。SDK `Record.shares` property + `grant_share()` / `revoke_share()` / `shared_with_emails()` + `SHARE_ROLES = ("viewer", "analyst")` 定数、`_to_dict()` で shares + shared_with_emails (sorted derived index) を保存、`_load_shares_dict()` で旧 record 互換 safe-fail。`app/permissions.py` 新設 (`can_read` / `can_analyze` / `can_grant` / `can_edit` の pure 関数 + `require_*` ヘルパ、team membership OR shares 経由で統一判定、SDK Record と dict の両方を duck-type で受ける)。`current_team_for_shared_access` + `get_lab_relaxed` (team membership 強制しない dep)。新 endpoint: `GET/POST/DELETE /api/records/{id}/shares`。既存 read endpoint (`GET /{id}`, `/children`, `/children/conditions`, `/cell_logs`) を `get_lab_relaxed` + `require_read` に切替で cross-team 閲覧解禁。observability event `record.share_granted` / `record.share_revoked`。grant 主体 = record.created_by + team admin、共有 grant の伝播禁止 (shares 経由は再共有不可)、record 自体の編集は team membership のみ。tests +25 件 (SDK 631→639 +8, backend 143→160 +17) |
