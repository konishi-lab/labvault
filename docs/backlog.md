...
最終更新: 2026-06-24 (PR #83 反映 — N4-N8 高優先 hot fix 完了)
...
### S1. 別チーム / 外部人への record 共有機能 — **次着手 (2026-06-24)**
...
**Phase 1 (4-5 日)**: 別チーム共有 (アカウント持ち、認証済みユーザー間)
- `shares: dict[email, role]` + `shared_with_emails: list[str]` フィールド
  (後者は Firestore array-contains 用の derived index)
- 新 endpoint: `POST /api/records/{id}/shares` (grant) /
  `DELETE /api/records/{id}/shares/{email}` (revoke) /
  `GET /api/records/shared-with-me` (list)
- 認可 dep 拡張: `can_user_read(user, record)` / `can_user_analyze(user, record)`
  を team membership OR shares 経由で統一判定
- Firestore index: `(deleted_at, shared_with_emails array-contains, updated_at DESC)`
- Frontend: 「共有」モーダル (email + role + active shares + 失効) +
  `/records?shared=1` で「他チームから共有された record」filter
...
Phase 1 で `can_user_*` 統一関数を作っておけば、Phase 2 は token を
仮想ユーザーとして同じ関数に流すだけになる。
