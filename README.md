...
- **ファイル管理**: `exp.add_file("data.ras")` で既存ファイル、`exp.add_object("plot.png", fig)` で Python オブジェクトを保存。装置ファイル(.ras, .dm3等)からメタデータを自動抽出
...
exp.add_object("fit_plot.png", fig)
```
