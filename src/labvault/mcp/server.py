...
def create_server(
    lab: Lab | None = None,
    *,
    lab_provider: Callable[[str | None], Lab] | None = None,
    stateless_http: bool = False,
) -> Any:
    """MCP サーバーを作成する。

    lab: 事前構築の Lab (主にテスト用)。渡された場合は lab.team の team として
    キャッシュに登録される。本番 (stdio) では None で OK (各ツールの team 引数 or
    LABVAULT_TEAM env から resolve した team で Lab を遅延構築する)。

    lab_provider: 渡されると built-in の team/Lab resolver の代わりにこれが使われる。
    引数はツール呼び出しの ``team`` 引数 (省略時 None)。リモート (Streamable HTTP)
    で per-request の認可・team 解決を外から注入するために使う。

    stateless_http: True にすると FastMCP の ``stateless_http`` 設定を立てる。
    Cloud Run のように複数インスタンスに分散される環境では True 推奨。
    """
    ...
    def _builtin_get_lab(team: str | None) -> Lab:
    ...
    def _get_lab(team: str | None = None) -> Lab:
        if lab_provider is not None:
            return lab_provider(team)
        return _builtin_get_lab(team)
